...
# Para abrir o arquivo resp.pkl
    ...
    while True:
        ...
        if key == ord ('n'):
            # Chame a função para criar um novo arquivo Excel com base no número da prova
            nome_arquivo = nome_arquivo_prova(numero_prova)
            numero_prova += 1  # Incremente o número da prova para o próximo arquivo

            # Se o arquivo ainda não existe, crie-o
            if not os.path.exists(nome_arquivo):
                workbook = openpyxl.Workbook()
                sheet = workbook.active
                sheet['A1'] = 'GABARITO'
                sheet['B1'] = 'GABARITO ALUNO'
                sheet['C1'] = 'RELAÇÃO'

                letras_gabarito = ['A', 'B', 'C', 'D', 'E']
                for i, letra in enumerate(letras_gabarito):
                    sheet.cell(row=i + 2, column=4, value=letra)
